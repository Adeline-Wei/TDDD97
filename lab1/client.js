...
sendSignUpInformation = function(){
    ...
    var infoObj = JSON.parse(JSON.stringify({
        'email':email,
        'password':password,
        'firstname':firstname,
    ...
    var sendBackMessage = serverstub.signUp(infoObj);
    if (sendBackMessage.success == true){
        // Sign in and display user's profile
        var sendBackMessage = serverstub.signIn(email, password);
        if (sendBackMessage.success == true){
			localStorage.setItem('token', sendBackMessage.data);		// UPDATE
            displayProfileView();
        }
    }
	else {
		document.getElementById("signUpAlert").innerHTML = sendBackMessage.success;
	}
};
